        ...
        for word_idx in range(0, len(sentence_tokens)):
            for tag_idx in range(0, len(self.tag_indexer)):
                if sentences[sentence_idx].tokens[word_idx].word not in stop_words: 
                    test_set_feature_cache[word_idx][tag_idx] = extract_emission_features(sentence_tokens, word_idx, self.tag_indexer.get_object(tag_idx), self.feature_indexer, add_to_indexer=False)
...
def train_crf_model(sentences):
    ...
    feature_weights = np.zeros(num_features)#[random.random() for i in range(num_features)]
    num_epochs = 3
    ...
    crfmodel = CrfNerModel(tag_indexer, feature_indexer, gradient_ascent.weights, transition_log_probs)
